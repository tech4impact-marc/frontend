...
import { ReportTypeSimpleResponseDto } from '@/pages/backOffice/report/[animal]'

interface ReportTypeProps {
  selectedAnimal: number
  responseType: ReportTypeSimpleResponseDto
  setResponseType: (value: React.SetStateAction<ReportTypeSimpleResponseDto>) => void
}
...
const ReportType: React.FC<ReportTypeProps> = ({
  ...
  const handleSubmit = () => {
    axios
      .patch(`${process.env.NEXT_PUBLIC_IP_ADDRESS}/reports/types/${selectedAnimal}`, responseType)
      .then((response) => {
  ...
  const handleDelete = () => {
    axios
      .delete(`${process.env.NEXT_PUBLIC_IP_ADDRESS}/reports/types/${selectedAnimal}`)
      .then((response) => {
        if (response.status == 200) {
          console.log(response)
          alert('설문이 삭제되었습니다')
          router.push(`/backOffice/report`)
        } else {
          console.log(response)
          alert('오류가 있었습니다')
        }
